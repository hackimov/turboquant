...
```python
turboquant_bits: float = Field(default=3, ge=1.5, le=4)
"""Bit width for TurboQuant KV when ``cache_dtype == \"turboquant\"`` (supports 1.5/2.5/3/4)."""
```
...
- **`EngineArgs`**: `turboquant_bits: float = get_field(CacheConfig, "turboquant_bits")`
